...
public class PersonaAdvisoryService {
    ...
    private PersonaAdvice generateDynamicPersonaAdvice(String symbol, com.tem.spring.ai.dto.UnifiedMarketContext context,
        ...
        String prompt = String.format("""
                ...
                ⑤ **엔지니어링 용어 노출 금지**:
                   - 'FastDTW', 'ta4j' 등 내부 개발 라이브러리 명칭은 답변에 절대 출력하지 마십시오.

                [Few-Shot Output 규격 예시]
                <thought>
                - 워런 버핏: 13F 현금비중과 기준가 괴리율을 고려해 안전마진 확보 권고.
                - 짐 시몬스: 시계열 프랙탈 승률 80%%와 RSI 모멘텀 기반 진입하되 SMA20 손절선 명시.
                ...
                <json>
                {
                  ...
                  "jimSimons": "시계열 프랙탈 과거 패턴 승률 80.0%%와 RSI 62.4 기준 통계적 우위 확인. 손익비 1:2.5 설정하되 SMA20 하향 돌파 시 즉시 기계적 손절을 집행하십시오.",
    ...
    private String fallbackSimons(QuantitativeSignal quant, com.tem.spring.ai.dto.UnifiedMarketContext context) {
        ...
        if (winRate >= 65.0) {
            return String.format("시계열 프랙탈 %.1f%% 일치 기반 과거 5봉 승률 %.0f%%(RSI %.1f) 확인. 르네상스 퀀트 모델 기준 손익비 1:2.5 진입 권고.",
                    sim, winRate, rsi);
        } else {
            return String.format("시계열 프랙탈 일치율(%.1f%%)의 통계적 우위가 모호함. 수수료와 슬리피지를 고려해 확실한 시그널까지 관망 요망.", sim);
        }
    }
